...
def scrape_papers(topic_url, output_json):
    print(f"Fetching main page: {topic_url}")
    ...
    for a in soup.find_all('a', href=True):
        ...
        elif re.search(r'\b(201[0-9]|202[0-9])\b', href):
    ...
    for i, (href, title) in enumerate(unique_threads):
        ...
        try:
            ...
            # If title is generic, grab it from the thread page itself
            if title.lower() in ('download', 'd', 'click here', ''):
    ...
    # Sort alphabetically
    results.sort(key=lambda x: x['thread_title'])

    with open(output_json, 'w', encoding='utf-8') as f:
        json.dump(results, f, indent=2, ensure_ascii=False)

    print(f"Scraping complete! Saved {len(results)} threads to {output_json}")

if __name__ == '__main__':
    # Scrape Class 10
    scrape_papers(
        topic_url='https://www.educationobserver.com/kerala-sslc-exam-model-previous-papers/',
        output_json='c:\\Users\\madhu\\code_test\\scraped_sslc.json'
    )
    
    # Scrape Class 11
    scrape_papers(
        topic_url='https://www.educationobserver.com/plus-one-annual-exam-model-papers-kerala-syllabus/',
        output_json='c:\\Users\\madhu\\code_test\\scraped_class11.json'
    )
    
    # Scrape Class 12
    scrape_papers(
        topic_url='https://www.educationobserver.com/model-previous-question-plus-two-exam-kerala/',
        output_json='c:\\Users\\madhu\\code_test\\scraped_class12.json'
    )
